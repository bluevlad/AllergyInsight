 ...
 *
 * 경쟁사 뉴스 모니터링 대시보드
 * 탭: 수집된 뉴스 | 실시간 검색 | 모니터링 업체 | 알러젠뉴스 정보
 */
...
const CompetitorNewsPage = () => {
  ...
  const [activeTab, setActiveTab] = useState('list'); // 'list', 'search', 'companies', 'allergen-news'
  ...
  return (
    <div className="news-page">
      ...
      <div className="tabs">
        ...
        <button
          className={`tab ${activeTab === 'allergen-news' ? 'active' : ''}`}
          onClick={() => setActiveTab('allergen-news')}
        >
          알러젠뉴스 정보
        </button>
      </div>
      ...
      {/* 알러젠뉴스 정보 탭 (read-only) */}
      {activeTab === 'allergen-news' && (
        <div className="tab-content">
          <AllergenNewsInfoTab />
        </div>
      )}
...
/**
 * 알러젠뉴스 정보 탭 (read-only)
 * 키워드 트렌드, 카테고리별 분석, 뉴스 트렌드 차트
 */
const AllergenNewsInfoTab = () => {
  ...
  if (!overview) return <p style={{ color: '#aaa', fontSize: '0.85rem', textAlign: 'center', padding: '2rem 0' }}>키워드 데이터가 없습니다. 키워드 추출을 먼저 실행해주세요.</p>;
    ...
    <div>
            ...
            {risingKeywords.map((kw, i) => (
              <span
                ...
                style={{
                  ...
                  padding: '0.4rem 0.75rem', color: 'white', borderRadius: '20px',
      ...
      <div className="an-cat-grid">
        {Object.entries(categories).map(([cat, keywords]) => (
          <div key={cat} className="an-card an-cat-card">
            ...
            {(keywords || []).length === 0 ? (
              <p style={{ color: '#aaa', fontSize: '0.85rem', textAlign: 'center', padding: '0.5rem 0' }}>키워드가 없습니다.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                {keywords.map((kw, i) => (
                  <div
                    ...
                    onMouseOver={e => { if (selectedKeyword !== kw.keyword) e.currentTarget.style.background = '#f8f9fa'; }}
                    onMouseOut={e => { if (selectedKeyword !== kw.keyword) e.currentTarget.style.background = 'transparent'; }}
                  >
                    <span style={{ fontSize: '0.85rem', fontWeight: 500 }}>{kw.keyword}</span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <span style={{ fontSize: '0.75rem', color: '#888' }}>{kw.mention_count}회</span>
      ...
      {selectedKeyword && (
        <div className="an-card" style={{ marginTop: '1.5rem' }}>
          ...
          {trendChartData.length > 0 ? (
            ...
            <p style={{ color: '#aaa', fontSize: '0.85rem', textAlign: 'center', padding: '1rem 0' }}>추이 데이터가 없습니다.</p>
          )}
      ...
      {Object.keys(categories).length === 0 && risingKeywords.length === 0 && (
        <p style={{ color: '#aaa', fontSize: '0.85rem', textAlign: 'center', padding: '2rem 0' }}>
          알러젠뉴스 데이터가 없습니다. 키워드 추출을 먼저 실행해주세요.
        </p>
      )}
      ...
      <style>{`
        ...
        .an-card-badge {
          background: #f0e6f6; color: #9b59b6;
